...
fix!(p::Parameter, par_names) = Symbol(p.name) ∈ par_names ? setfield!(p, :fixed, true) : nothing
...
release!(p::Parameter, par_names) = Symbol(p.name) ∈ par_names ? setfield!(p, :fixed, false) : nothing
